...
$assert(strpos($orders, '$offset = ($current_page - 1) * $items_per_page;') !== false, 'Page 3 must use a different server offset from page 1.');
$assert(strpos($orders, '$current_page = max(1, (int)($_GET[\'page\'] ?? 1));') !== false, 'Missing and invalid page input must resolve safely.');
...
$assert(strpos($orders, 'data-turbo="false"') !== false, 'Orders pagination must remain outside Turbo interception.');
...
$assert(strpos($orders, 'event.preventDefault();') !== false && strpos($orders, '{ targetUrl: targetUrl }') !== false, 'Pagination clicks must use the guarded Orders AJAX request instead of relying on a second navigation architecture.');
$assert(strpos($orders, "window.location.assign(targetUrl);") !== false, 'A failed pagination AJAX request must fall back to the canonical page URL.');
...
$assert(substr_count($orders, 'class="orders-card-actions') >= 2, 'Initial and AJAX rows must use the dedicated equal-width action grid.');
$assert(strpos($orders, 'class="action-cell orders-card-actions') === false, 'Orders actions must not inherit the shared flex action-cell geometry.');
...
if (function_exists('render_pagination')) {
    ...
    $assert(strpos($first, 'data-page="2"') !== false && strpos($first, 'data-turbo="false"') !== false, 'Rendered controls must expose the intended page and bypass Turbo.');
