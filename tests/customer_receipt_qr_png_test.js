...
const match = source.match(/function receiptQrContrastMetrics\(pixels\) \{[\s\S]*?\r?\n\}\r?\n\r?\nfunction receiptQrPngDataUrl\(payload\) \{[\s\S]*?\r?\n\}\r?\n\r?\nfunction receiptWaitForImages/);
...
(async () => {
    ...
    assert.deepStrictEqual(operations[0], ['fillRect', 0, 0, 396, 396]);
    assert.deepStrictEqual(operations[1], ['drawImage', qrCanvas, 48, 48]);
