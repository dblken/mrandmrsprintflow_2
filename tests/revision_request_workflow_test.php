...
revision_test_assert(strpos($orders, 'function openRevisionForm(') === false, 'obsolete preventDefault revision navigation handler is removed');
...
revision_test_assert(strpos($editOrder, 'LEFT JOIN branches b ON b.id = o.branch_id') !== false && strpos($editOrder, 'b.branch_id = o.branch_id') === false, 'revision form uses the production branches.id schema');
revision_test_assert(strpos($editOrder, '<input type="hidden" name="resubmit_order" value="1">') !== false, 'disabled submit button cannot remove the revision POST action');
...
revision_test_assert(strpos($workflow, 'printflow_revision_keys_are_verified_aliases') !== false && strpos($workflow, 'equivalentKeys') !== false, 'only verified canonical and human-label specification aliases are synchronized');
revision_test_assert(strpos($editOrder, 'customer_revision_dedupe_specs') !== false && strpos($editOrder, 'customer_revision_scalar_text($value) !==') !== false, 'duplicate Print Type aliases render as one logical non-empty control');
revision_test_assert(substr_count($editOrder, 'printflow_resolve_service_catalog_service_id_for_order_line') >= 2 && strpos($workflow, 'printflow_resolve_service_catalog_service_id_for_order_line($custom, $order, $item)') !== false, 'legacy service order lines use verified catalog field configuration');
revision_test_assert(strpos($staffCustomizations, 'staffCollectRevisionFieldCandidates') !== false && strpos($staffCustomizations, 'staffBuildItemDisplaySpecs') !== false, 'staff revision checklist derives from the same display-spec pipeline as Order Details');
revision_test_assert(strpos($staffCustomizations, 'staffRevisionResolveDesignUploadField') !== false && strpos($staffCustomizations, 'staffRevisionDesignCurrentValue') !== false, 'staff revision design upload resolves from service field metadata');
revision_test_assert(strpos($staffCustomizations, 'revisionDesignReasonUnavailable') !== false, 'design-related reasons surface a clear message when no upload field exists');
revision_test_assert(strpos($staffCustomizations, 'staffRevisionPermissionToken') !== false && strpos($staffCustomizations, 'spec:${itemId}:') !== false, 'staff revision permissions use stable field identifiers and spec tokens');
revision_test_assert(strpos($staffCustomizations, '<option value="invalid_format">') === false, 'invalid format is removed from new staff revision reason choices');
revision_test_assert(strpos($staffCustomizations, 'Other: ${this.getCustomLabel(key)}') === false, 'staff revision checklist no longer appends generic Other: metadata labels');
revision_test_assert(strpos($staffCustomizations, 'Details customer needs to update') !== false && strpos($staffCustomizations, 'Select at least one detail the customer needs to update.') !== false, 'staff revision modal requires at least one editable field');
