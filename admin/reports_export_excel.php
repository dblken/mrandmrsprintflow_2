...
if ($report === 'orders') {
    ...
    buildDailySalesReport($sheet, $day, $branchName, $branchId, $jobCompletedSql);
...
function buildSalesReport($sheet, $branchName, array $salesBundle) {
    ...
    $row++;
    foreach ([['Total Sales Transactions', $totalOrd, false], ['Total Revenue', $totalRev, true], ['Product Sales', (float)($sum['product_sales'] ?? 0), true], ['Service/Custom Sales', (float)($sum['service_sales'] ?? 0), true], ['Average Sale Value', $avgVal, true]] as $line) {
        $sheet->setCellValue('A' . $row, $line[0]);
        $sheet->setCellValue('B' . $row, $line[1]);
        $sheet->getStyle('A' . $row)->getFont()->setBold(true);
        $sheet->getStyle('B' . $row)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
        if ($line[2]) $sheet->getStyle('B' . $row)->getNumberFormat()->setFormatCode('"PHP" #,##0.00');
        $row++;
    }
    $row += 2;
...
function buildDailySalesReport($sheet, string $day, string $branchName, $branchId, string $jobCompletedSql): void {
    ...
    $paidOnly = pf_reports_store_order_paid_completed_expr('o');
    ...
    if ($branchId !== 'all') {
        $orders = db_query(
            "SELECT o.order_id,
                    ...
                    o.order_date, {$storeRevenue} AS total_amount, o.status, o.payment_status
    ...
    if ($branchId !== 'all') {
        $sOrders = db_query(
            "SELECT jo.id,
                    ...
                    jo.status
             FROM job_orders jo LEFT JOIN customers c ON jo.customer_id = c.customer_id
    ...
    $sheet->setCellValue('A' . $row, 'SERVICE JOBS (paid/completed)');
    $sheet->getStyle('A' . $row)->getFont()->setBold(true);
    $row += 2;
    $svcHeader = $row;
