...
try {
    ...
    if ($isCustomers) {
        $customerStoreSalesSql = pf_reports_store_order_sales_expr('o');
        $customerStoreRevenueSql = pf_reports_store_order_revenue_expr('o');
    ...
    $paidOrders = $grandTotalOrd;
    $avgOrderVal = $grandTotalOrd > 0 ? ($grandTotalRev / $grandTotalOrd) : 0;
        ...
        try {
            ...
            if ($cursor !== false && $end !== false) {
                while ($cursor <= $end) {
                    ...
                    $dayData = pf_reports_official_sales_breakdown($day . ' 00:00:00', $day . ' 23:59:59', $branchId, 1);
                    $daySummary = $dayData['summary'] ?? [];
                    $dayCount = (int)($daySummary['transaction_count'] ?? 0);
                    $dayRevenue = (float)($daySummary['total_sales'] ?? 0);
                    if ($dayCount > 0 || $dayRevenue > 0) {
                        $daily_sales[] = ['day' => $day, 'cnt' => $dayCount, 'revenue' => $dayRevenue];
                    }
    ...
    if ($show_insights) {
        ...
        $prevSalesData = pf_reports_official_sales_breakdown($prevFrom, $prevToEnd, $branchId, 1);
