...
function pf_reports_store_order_paid_completed_expr(string $alias = 'o'): string {
    ...
    return pf_reports_store_order_sales_expr($alias);
}
...
function pf_reports_job_order_paid_completed_expr(string $alias = 'jo'): string {
    ...
    return pf_reports_job_order_sales_expr($alias);
}
...
function pf_reports_job_order_revenue_expr(string $alias = 'jo'): string {
    $alias = pf_reports_sql_alias($alias, 'jo');
    return "COALESCE(NULLIF({$alias}.amount_paid, 0), {$alias}.estimated_total, 0)";
}

/** Official customization/job-order sales date. */
function pf_reports_job_order_sales_date_expr(string $alias = 'jo'): string {
    $alias = pf_reports_sql_alias($alias, 'jo');
    return "COALESCE({$alias}.payment_verified_at, {$alias}.created_at)";
}

/**
 * Date-range SQL for a trusted expression. Caller provides only internal SQL.
 *
 * @return array{0:string,1:string,2:array<int,string>}
 */
function pf_reports_date_expr_where(string $expr, string $from, string $toEnd): array {
    if ($from !== '' && $toEnd !== '') {
        return [" AND {$expr} BETWEEN ? AND ?", 'ss', [$from, $toEnd]];
    }
    if ($from !== '') {
        return [" AND {$expr} >= ?", 's', [$from]];
    }
    if ($toEnd !== '') {
        return [" AND {$expr} <= ?", 's', [$toEnd]];
    }
    return ['', '', []];
}

/**
 * Shared official sales dataset used by dashboard, breakdown, print, CSV, and Excel.
 *
 * @return array{summary:array<string,mixed>,by_branch:list<array<string,mixed>>,by_item:list<array<string,mixed>>,transactions:list<array<string,mixed>>}
 */
function pf_reports_official_sales_breakdown(string $from, string $toEnd, $branchId, int $transactionLimit = 100): array {
    $summary = [
        'total_sales' => 0.0,
        'transaction_count' => 0,
        'product_sales' => 0.0,
    ...
    $storeSales = pf_reports_store_order_sales_expr('o');
    $storeRevenue = pf_reports_store_order_revenue_expr('o');
    $jobSales = pf_reports_job_order_sales_expr('jo');
    $jobRevenue = pf_reports_job_order_revenue_expr('jo');
    $jobDate = pf_reports_job_order_sales_date_expr('jo');
    $hasStoreAmountPaid = function_exists('db_table_has_column')
        ? db_table_has_column('orders', 'amount_paid')
        : pf_reports_table_has_column('orders', 'amount_paid');
    $hasStorePaymentMethod = function_exists('db_table_has_column')
        ? db_table_has_column('orders', 'payment_method')
        : pf_reports_table_has_column('orders', 'payment_method');
    $hasJobPaymentMethod = function_exists('db_table_has_column')
        ? db_table_has_column('job_orders', 'payment_method')
        : pf_reports_table_has_column('job_orders', 'payment_method');
    $storePaymentMethod = $hasStorePaymentMethod
        ? "CASE WHEN LOWER(TRIM(COALESCE(o.payment_method, ''))) = 'cash' THEN 'Cash' ELSE 'QR Ph' END"
        : "'QR Ph'";
    ...
    try {
        ...
        $row = db_query("SELECT COUNT(*) AS cnt, COALESCE(SUM({$jobRevenue}), 0) AS rev FROM job_orders jo WHERE {$jobSales}{$dj}{$bj}", $dtj . $btj, array_merge($dpj, $bpj))[0] ?? [];
        $summary['service_transactions'] = (int)($row['cnt'] ?? 0);
        $summary['service_sales'] = (float)($row['rev'] ?? 0);
    } catch (Throwable $e) {}

    $summary['total_sales'] = round((float)$summary['product_sales'] + (float)$summary['service_sales'], 2);
    $summary['transaction_count'] = (int)$summary['product_transactions'] + (int)$summary['service_transactions'];
    $summary['product_sales'] = round((float)$summary['product_sales'], 2);
    $summary['service_sales'] = round((float)$summary['service_sales'], 2);
    ...
    try {
        [$bj, $btj, $bpj] = branch_where_parts('jo', $branchId);
        [$dj, $dtj, $dpj] = pf_reports_date_expr_where($jobDate, $from, $toEnd);
        $rows = db_query("SELECT jo.branch_id, COALESCE(b.branch_name, CONCAT('Branch #', jo.branch_id)) AS branch_name, COUNT(*) AS service_transactions, COALESCE(SUM({$jobRevenue}), 0) AS service_sales FROM job_orders jo LEFT JOIN branches b ON b.id = jo.branch_id WHERE {$jobSales}{$dj}{$bj} GROUP BY jo.branch_id, b.branch_name", $dtj . $btj, array_merge($dpj, $bpj)) ?: [];
        foreach ($rows as $r) {
            $key = (int)($r['branch_id'] ?? 0);
            if (!isset($byBranch[$key])) {
                $byBranch[$key] = ['branch_id' => $key, 'branch_name' => (string)($r['branch_name'] ?? ('Branch #' . $key)), 'product_sales' => 0.0, 'service_sales' => 0.0, 'product_transactions' => 0, 'service_transactions' => 0];
            }
            $byBranch[$key]['service_sales'] += (float)($r['service_sales'] ?? 0);
            $byBranch[$key]['service_transactions'] += (int)($r['service_transactions'] ?? 0);
        }
    } catch (Throwable $e) {}

    foreach ($byBranch as &$branch) {
        $branch['total_sales'] = round((float)$branch['product_sales'] + (float)$branch['service_sales'], 2);
        $branch['transaction_count'] = (int)$branch['product_transactions'] + (int)$branch['service_transactions'];
        $branch['product_sales'] = round((float)$branch['product_sales'], 2);
        $branch['service_sales'] = round((float)$branch['service_sales'], 2);
    }
    unset($branch);
    $byBranch = array_values($byBranch);
    usort($byBranch, static fn($a, $b) => (($b['total_sales'] ?? 0) <=> ($a['total_sales'] ?? 0)));
    ...
    try {
        [$bj, $btj, $bpj] = branch_where_parts('jo', $branchId);
        [$dj, $dtj, $dpj] = pf_reports_date_expr_where($jobDate, $from, $toEnd);
        $rows = db_query("SELECT 'Service' AS type, COALESCE(NULLIF(TRIM(jo.service_type), ''), NULLIF(TRIM(jo.job_title), ''), 'Customization') AS item_name, COALESCE(SUM(COALESCE(jo.quantity, 1)), 0) AS quantity, COALESCE(SUM({$jobRevenue}), 0) AS revenue FROM job_orders jo WHERE {$jobSales}{$dj}{$bj} GROUP BY COALESCE(NULLIF(TRIM(jo.service_type), ''), NULLIF(TRIM(jo.job_title), ''), 'Customization')", $dtj . $btj, array_merge($dpj, $bpj)) ?: [];
        foreach ($rows as $r) {
            $byItem[] = ['type' => 'Service', 'item_name' => (string)($r['item_name'] ?? 'Customization'), 'quantity' => (float)($r['quantity'] ?? 0), 'revenue' => round((float)($r['revenue'] ?? 0), 2)];
        }
    } catch (Throwable $e) {}
    usort($byItem, static fn($a, $b) => (($b['revenue'] ?? 0) <=> ($a['revenue'] ?? 0)));

    $limit = max(1, min(500, $transactionLimit));
    $txnRows = [];
    try {
        [$bo, $bto, $bpo] = branch_where_parts('o', $branchId);
        [$do, $dto, $dpo] = pf_reports_date_expr_where('o.order_date', $from, $toEnd);
...
/** Normalized category label for sticker split matching. */
...
/** Rows aggregated under catalog name Sticker / Stickers (source pool for the split). */
...
/**
 * Maps an existing row to a recipient bucket (excluding sticker — that share is applied separately).
 *
...
/**
 * Split pooled "Sticker(s)" revenue evenly across six presentation buckets so one wedge is not
 * inflated: one sixth stays as Stickers; one sixth each goes to Available Products, Signage,
 * Sintraboard, T-Shirt, and Merchandise (creating slices where missing). Overall totals unchanged.
 *
 * @param array<int, array<string,mixed>> $rows
 * @return array<int, array<string,mixed>>
 */
function pf_reports_split_sticker_sales_including_stickers_bucket(array $rows): array {
    if ($rows === []) {
        return [];
    }
    ...
    $allBuckets = ['sticker', 'available', 'signage', 'sintraboard', 'tshirt', 'merchandise'];
    $labels = [
        'sticker' => 'Stickers',
...
function pf_reports_sales_by_product_category(string $from, string $toEnd, $branchId): array {
    ...
    try {
        ...
        $rows = pf_reports_split_sticker_sales_including_stickers_bucket($rows);
...
function pf_reports_heatmap_available_years($branchId): array {
    ...
    try {
        [$bj, $btj, $bpj] = branch_where_parts('jo', $branchId);
        $jrows = db_query(
            "SELECT DISTINCT YEAR({$jobDate}) AS y
             FROM job_orders jo
             WHERE {$jobSales}
               AND YEAR({$jobDate}) <= YEAR(CURDATE())$bj",
            $btj ?: null,
            $bpj ?: null
        ) ?: [];
        foreach ($jrows as $r) {
        ...
        $jobRaw = db_query(
            "SELECT COALESCE(NULLIF(TRIM(jo.service_type), ''), 'Customization') AS product,
             ...
             WHERE {$jobSales}
               AND YEAR({$jobDate}) = ?
               AND (
                 YEAR({$jobDate}) < YEAR(CURDATE())
                 OR MONTH({$jobDate}) <= MONTH(CURDATE())
               )$bj
             GROUP BY COALESCE(NULLIF(TRIM(jo.service_type), ''), 'Customization'), MONTH({$jobDate})
             ORDER BY product, mo",
            $jTypes,
            $jParams
        ) ?: [];
...
function pf_reports_top_products_merged(string $from, string $toEnd, $branchId, int $limit = 10): array {
    ...
    try {
        ...
        $jdTypes = "";
        if ($from !== '' && $toEnd !== '') {
            $jDatePart = " AND {$jobDate} BETWEEN ? AND ?";
            $jdParams = [$from, $toEnd];
            $jdTypes = "ss";
        } elseif ($from !== '') {
            $jDatePart = " AND {$jobDate} >= ?";
            $jdParams = [$from];
            $jdTypes = "s";
        } elseif ($toEnd !== '') {
            $jDatePart = " AND {$jobDate} <= ?";
        ...
        $jrows = db_query(
            "SELECT COALESCE(NULLIF(TRIM(jo.service_type), ''), 'Customization') AS svc,
                    ...
                    SUM(CASE WHEN {$jobSales} THEN {$jobRevenue} ELSE 0 END) as revenue
             FROM job_orders jo
    ...
    usort($list, static function ($a, $b) {
        $rev = ($b['revenue'] ?? 0) <=> ($a['revenue'] ?? 0);
        return $rev !== 0 ? $rev : (($b['qty_sold'] ?? 0) <=> ($a['qty_sold'] ?? 0));
    });
...
function pf_reports_branch_performance_merged(string $from, string $toEnd, $branchId = 'all', string $prevFrom = null, string $prevToEnd = null): array {
    ...
    try {
        ...
        $oRows = db_query(
            "SELECT o.branch_id,
                    SUM(CASE WHEN {$storeSales} THEN 1 ELSE 0 END) AS ord,
                    SUM(CASE WHEN {$storeSales} THEN {$storeRevenue} ELSE 0 END) AS rev
             FROM orders o
    ...
    try {
        ...
        $jRows = db_query(
            "SELECT jo.branch_id,
                    SUM(CASE WHEN {$jobSales} THEN 1 ELSE 0 END) AS ord,
                    SUM(CASE WHEN {$jobSales} THEN {$jobRevenue} ELSE 0 END) AS rev
             FROM job_orders jo
    ...
    if ($prevFrom && $prevToEnd) {
        try {
            ...
            $oRows = db_query(
                "SELECT o.branch_id, SUM(CASE WHEN {$storeSales} THEN {$storeRevenue} ELSE 0 END) AS rev
                 FROM orders o
                 WHERE o.branch_id IS NOT NULL {$oDatePart}
                 GROUP BY o.branch_id",
                $odTypes,
            ...
            $jRows = db_query(
                "SELECT jo.branch_id, SUM(CASE WHEN {$jobSales} THEN {$jobRevenue} ELSE 0 END) AS rev
                 FROM job_orders jo
                 WHERE jo.branch_id IS NOT NULL {$jDatePart}
                 GROUP BY jo.branch_id",
                $jdTypes,
...
function pf_reports_period_sales_merged(string $from, string $toEnd, $branchId): array {
    ...
    try {
        ...
        $fmt = ($groupBy === 'MONTH') ? '%Y-%m' : '%Y-%m-%d';
        $sql = "SELECT DATE_FORMAT({$jobDate},'{$fmt}') as d,
                       COUNT(*) as cnt,
                       SUM(CASE WHEN {$jobSales} THEN {$jobRevenue} ELSE 0 END) as rev
