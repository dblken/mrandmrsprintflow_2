...
<head>
    ...
    function renderOrderModal(d) {
        ...
        if (!isViewOnlyStatus) {
            if (isVerifyStatus) {
                if (canVerifyPayment) {
                    actionsHTML = '<div style="margin-top:28px; display:grid; gap:12px; justify-items:center;">' +
                        '<button class="btn-primary" onclick="verifyPaymentProof(' + d.order_id + ', \'Approve\')" style="width:auto; max-width:220px; min-width:140px; justify-self:center; background:#059669; color:white; border:none; padding:12px 16px; border-radius:10px; font-weight:700; cursor:pointer; font-size:14px;">Approve</button>' +
                        '<button class="btn-primary" onclick="verifyPaymentProof(' + d.order_id + ', \'Reject\')" style="width:auto; max-width:220px; min-width:140px; justify-self:center; background:#ef4444; border-color:#ef4444; color:white; border:none; padding:12px 16px; border-radius:10px; font-weight:700; cursor:pointer; font-size:14px;">Reject</button>' +
                        '</div>';
                } else {
