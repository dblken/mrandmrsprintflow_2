...
if (!is_logged_in()) {
    $respond(['success' => false, 'message' => 'Your session has expired. Please sign in and scan again.'], 401);
}

$userType = (string)get_user_type();
if (!in_array($userType, ['Staff', 'Manager', 'Admin'], true)) {
    $respond(['success' => false, 'message' => 'You are not authorized to look up receipt orders.'], 403);
}
...
if ($identifier === '' || $orderId <= 0) {
    $respond(['success' => false, 'message' => 'Invalid receipt QR or order identifier. Please verify the receipt and scan again.'], 422);
}
...
try {
    ...
    if (empty($rows)) {
        $respond(['success' => false, 'message' => 'Order not found. Please verify the receipt and scan again.'], 404);
    }
    ...
    if ($userType !== 'Admin') {
        ...
        if (!is_array($allowedBranches) || !in_array($orderBranchId, array_map('intval', $allowedBranches), true)) {
            $respond(['success' => false, 'message' => 'This order belongs to another branch. Ask an authorized user from that branch for assistance.'], 403);
        }
    }

    if ($userType === 'Staff' && !printflow_staff_role_can_access_order_source(printflow_get_staff_access_role(), $orderSource)) {
        $respond(['success' => false, 'message' => 'This order belongs to a different staff operation. Please ask the appropriate counter or online staff team for assistance.'], 403);
    }
    ...
    error_log('[order-receipt-lookup] Lookup failed for order #' . $orderId . ': ' . $e->getMessage());
    $respond(['success' => false, 'message' => 'Order lookup is temporarily unavailable. Please try again.'], 500);
}
