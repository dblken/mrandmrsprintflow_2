...
foreach ($catalog_entries as $entry) {
    if (($entry['type'] ?? '') === 'group') {
        ...
        if ($gid > 0) {
            $group_ids_on_page[] = $gid;
        }
    }
}
$group_stats_map = printflow_catalog_group_card_stats_map($group_ids_on_page);
            ...
            <div class="shopee-grid">
                <?php foreach ($catalog_entries as $product_index => $entry):
                    if (($entry['type'] ?? '') === 'group'):
                        ...
                        $groupStats = $group_stats_map[$groupId] ?? ['avg_rating' => 0.0, 'review_count' => 0, 'sold_count' => 0];
                ...
                ?>
                    <div class="shopee-card shopee-card-group" onclick="window.location.href='product_group.php?group_id=<?php echo $groupId; ?>'">
                        ...
                        <div class="shopee-body">
                            ...
                            <div class="rating-stars">
                                ...
                                <?php endfor; ?>
                                <span class="rating-text"><?php echo $review_count > 0 ? '(' . (int) $review_count . ')' : ''; ?></span>
